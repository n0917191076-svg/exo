...
本檔案是給 Claude Code 的完整開發指南。開工前先把本檔案全部讀完，再讀 `KNOWN_QUIRKS.md`。
...
2. **重寫 `modes.ts`**：刪除 date/sting/sales/listen，保留結構。新模式：
   ...
   每個 systemPrompt 用繁體中文撰寫，並包含共同規則：「建議 2–3 條，每條可直接照著念出口；先結論；不加前言；逐條換行」。
3. **場景說明（scene note）**：settings 新增一個文字欄「目前場景」（例：面試 / 簡報 / 會議＋一句補充）。/suggest 時原樣附進 prompt：`【目前場景】...`。
4. **模型選擇**：settings 下拉 `claude-haiku-4-5`（快）/ `claude-sonnet-4-6`（預設，聰明）。傳給 Worker，Worker 轉給 Anthropic API 的 `model` 欄位。
5. **回答長度**：settings 三選一 短/中/長 → prompt 附加：「每條 ≤10 字 / ≤20 字 / ≤40 字」。
6. **語言模式**：settings 切換 中文/英文。英文模式的 prompt 分支：「先輸出一行『譯：<對方那句話的中文翻譯>』，再給 2 條英文回答建議（簡單詞彙，CEFR B1 內）」。
...
1. **新模式 `solve`（直答）glyph `✦`**：語意翻轉——閘門收到的聲音視為「使用者本人的提問」，prompt 目標是**直接回答問題本身**，不是建議怎麼回話。回答格式：答案先行（第一行就是答案/結論），之後最多 2–3 行關鍵步驟或理由。程式/數學題：先給最終答案，再給關鍵思路，不逐行列程式碼（眼鏡念不動）。
