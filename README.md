...
Even Realities G2 智慧眼鏡上的即時對話輔助。對方說話 → 轉文字 → Claude 依「模式＋場景＋知識庫」生成一段可直接照念的答案 → 逐字顯示在鏡片上，照著講就行。
...
3. **對話設定**：語言（中文／英文＝一行「譯：中文翻譯」＋一段英文回答）、模型（Sonnet 聰明／Haiku 快）、回答長度（中文 短40–70／中80–110／長110–140 字；英文 20–30／35–50／50–70 words）、目前場景（例：「面試——金融後台主管面」，會原樣進 prompt）。
...
|---|---|---|
| 單擊 | 待命 | 開始收音（對方開始說話時按） |
| 單擊 | 收音中 | 結束收音＋生成回答（對方說完時按） |
| 雙擊 | 收音中 | 取消本段（誤觸時用，丟棄不送） |
| 雙擊 | 回答顯示中 | **延伸**：接續深入一層（可連按逐層加深） |
| 單擊 | 回答顯示中 | 開始新一輪（清屏） |
| 雙擊 | 純待命（無回答） | 離開 App |

手機大按鈕：**按住收音、放開送出、滑出取消**（可盲按）。模式切換只在手機（工作■／日常●／自訂★）。答案串流逐字上屏——第一批字出現得比完整生成早；眼鏡定錨在答案開頭、由你的朗讀節奏推進（長度已壓短對齊單一視窗，長答案在手機端看全文）。
