# AGENTS.md — Exo（AI 外骨骼）

本檔案是給 Codex 的完整開發指南。開工前先把本檔案全部讀完，再讀 `KNOWN_QUIRKS.md`。**回答格式一律以 `CLAUDE.md` 的「## 回答輸出契約」為唯一真相，不在本檔重複維護**（單一答案設計，非 2–3 條建議）。

## 這個專案是什麼

**Exo** — 使用者 Evan 的個人「AI 外骨骼」：Even Realities G2 智慧眼鏡上的即時對話輔助 App。
眼鏡麥克風收音 → Deepgram 轉文字（中文為主）→ Codex 依「模式 + 場景 + 知識庫」生成建議回答 → 逐字顯示在眼鏡鏡片上，Evan 照著講。
...
2. **重寫 `modes.ts`**：刪除 date/sting/sales/listen，保留結構。新模式：
   ...
   每個 systemPrompt 用繁體中文撰寫，共同規則見 `CLAUDE.md`「## 回答輸出契約」（單一完整答案，非 2–3 條）。
3. **場景說明（scene note）**：settings 新增一個文字欄「目前場景」（例：面試 / 簡報 / 會議＋一句補充）。/suggest 時原樣附進 prompt：`【目前場景】...`。
4. **模型選擇**：settings 下拉 `Codex-haiku-4-5`（快）/ `Codex-sonnet-4-6`（預設，聰明）。傳給 Worker，Worker 轉給 Anthropic API 的 `model` 欄位。
5. **回答長度**：settings 三選一 短/中/長 → 依 `CLAUDE.md`「## 回答輸出契約」長度表附加（中文 80–120／180–240／320–420 字；英文 30–45／70–100／130–170 words）。
6. **語言模式**：settings 切換 中文/英文。英文模式：第一行『譯：<對方那句話的中文翻譯>』，空一行後給**一個**完整英文回答（CEFR B1 內）。詳見 `CLAUDE.md`「## 回答輸出契約」。
...
1. **新模式 `solve`（直答）glyph `✦`**：語意翻轉——閘門收到的聲音視為「使用者本人的提問」，prompt 目標是**直接回答問題本身**，不是建議怎麼回話。回答格式：答案先行（第一行就是答案/結論），之後最多 2–3 行關鍵步驟或理由。程式/數學題：先給最終答案，再給關鍵思路，不逐行列程式碼（眼鏡念不動）。（solve/guide/talk 的輸出格式獨立於單一答案結構，但沿用 `CLAUDE.md`「## 回答輸出契約」的「內容與事實政策」。）
...
2. **模式 `talk`（即興演講/申論）glyph `◉`**：我給一個議題，AI 立刻給第一段，我開始照念的同時它持續生成下文，永遠供稿在我前面。
   - 實作本質＝**一次長串流＋分頁緩衝**：Codex 生成速度遠快於朗讀速度，客戶端把串流內容切成頁緩衝，**下滑＝下一段、上滑＝回看**；緩衝進度用認證字元（如 `▇▇▇▁▁▁▁` 或「3/7」）。
...
- Even Hub 開發文件：hub.evenrealities.com/docs（含 Codex 專頁 /docs/AI-tooling/Codex、Device APIs、Networking、Background & Lifecycle）
...
- Anthropic API：docs.Codex.com（Messages API、streaming）
...
- Codex Haiku 每次建議約 <NT$0.1；Sonnet 約其 3–5 倍
